...
requires a solution of costate variables that relate a change in the flow
equation residuals to a change in the function of interest.  The adjoint solver
also suffers from the quadratic scaling in computational cost and memory
required, similar to the primal flow solver.  To mitigate this, a decoupled
scheme is derived that is consistent with the decoupled flow solver.
...
\end{equation}
%------------------------------------------------------------------------------%
The details and derivation of the transformation matrices need to convert
...
\begin{equation}
  \begin{aligned}
    \left(\mm \ma_{\mU} \right) \left( \mb \Delta \mv \right) &= \mm \ru{} \\
...
scheme become a very important boon in cases involving many species, since the
memory freed by the sparsity on the LHS of \eref{adj-u-to-v-time} can be used by
the RHS linearizations to increase in computational efficiency.

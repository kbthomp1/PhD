...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<3-> This transformation avoids having to explicitly form the jacobian
      $\pd{\rv{}}{\mv}$, which are complicated than $\pd{\ru{}}{\mU}$
  \end{itemize}
\end{frame}
\begin{frame}
  \frametitle{Decoupled Adjoint Scheme}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Time march adjoint solution with approximate jacobians
      \begin{equation*}
        \left( \frac{V}{\Delta t} \mi + \pd{\rv{1}}{\mv}^T \right) \adjlam{\mv}
        =
        - \left( \pd{\mU}{\mv} \right)^T
        \underbrace{
          \left( \pd{\ru{2}}{\mU} \adjlam{\mU} + \pd{f}{\mU} \right)
        }_\text{Fully Coupled Residual}
      \end{equation*}
    \item Split $\pd{\rv{1}}{\mv}$ in same fashion as flow solver
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Separate into two systems and solve as block jacobi scheme
      \begin{smallequation}
        \begin{cboxn}[red]
          \left( \frac{\rho V}{\Delta t} \mi + \rdiff{\mv_i}{c_s} \right) 
          \Delta \adjlam{\mv_{c_i}}
          = -\rho \left( 
            \pd{f}{\rho_s} + \rdiff{\mU}{\rho_s} \adjlam{\mU}
          \right)
        \end{cboxn}
      \end{smallequation}
      \begin{smallequation}
        \begin{cboxn}[blue]
          \begin{split}
            \left[
              \frac{V}{\Delta t} \mi +
              \begin{pmatrix}
                \tdiff{\rho}{\rho} & \tdiff{\rho}{\rho \vu} 
                & \tdiff{\rho}{\rho E} \\ \\
                \tdiff{\rho \vu}{\rho} & \tdiff{\rho \vu}{\rho \vu} 
                & \tdiff{\rho \vu}{\rho E} \\ \\
                \tdiff{\rho E}{\rho} & \tdiff{\rho E}{\rho \vu} 
                & \tdiff{\rho E}{\rho E}
              \end{pmatrix}
            \right]
            \begin{pmatrix}
              \Delta \adjlam{\mv_{\rho}} \\ \\
              \Delta \adjlam{\mv_{\rho \vu}} \\ \\
              \Delta \adjlam{\mv_{\rho E}}
            \end{pmatrix}
            &= \\ -
            \begin{pmatrix}
              \pd{f}{\rho} \\ \\

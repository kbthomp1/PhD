...
advantagous to generate structured-type grids to preserve grid alignment with
...
grids are unstructured, consisting totally of hexahedral elements with a single
...
\end{equation} 
%------------------------------------------------------------------------------%
Substituting Eq.~(\ref{f3d_off_diag}) into
Eq.~(\ref{mem_req_eq}), the relative memory cost is:
%------------------------------------------------------------------------------%
\begin{equation} 
  Relative\ Memory\ Cost = 
  \frac{N_{nodes}}{N_{nodes} + N_{nz}} =
...
hexahedral grid was constructed, and freestream conditions were set as
...
improve the effiency of high-velocity simulations, and that the stiffness of
